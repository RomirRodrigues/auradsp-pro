...
// 1. Rebranding
html = html.replace(/boAt audio devices/g, 'Aura Audio Gear');
html = html.replace(/Dolby Virtualizer/g, 'Aura Virtualizer');
html = html.replace(/boAt Audio Preset Active/g, 'Aura Reference Active');
html = html.replace(/data-cat="boat">boAt Audio</g, 'data-cat="boat">Aura Tuned</g');
html = html.replace(/boAt Signature Sound/g, 'Aura Signature Sound');
html = html.replace(/boAt Rockerz & Airdopes/g, 'Aura Wireless & Earbuds');
html = html.replace(/boAt Bassheads Ultra Boost/g, 'Aura Bassheads Ultra Boost');
html = html.replace(/boAt Beast™ Low-Latency Gaming/g, 'Aura Beast™ Low-Latency Gaming');
html = html.replace(/boAt Stone Soundbar Mode/g, 'Aura Soundbar Mode');
html = html.replace(/boAt Bluetooth soundbars/g, 'Aura Soundbars');
html = html.replace(/Dolby Dynamic Range Compressor/g, 'Aura Dynamic Range Compressor');
html = html.replace(/Dolby Night & Dynamic Control/g, 'Dynamic Night Mode & Vocal Control');
html = html.replace(/Dolby 7.1 Spatial Audio & Vocal Test/g, '7.1 Spatial Audio & Vocal Test');
html = html.replace(/DOLBY VIRTUAL SOUND & SOUNDFIELD ENHANCERS/g, 'AURA DYNAMIC SOUND & SOUNDFIELD ENHANCERS');
...
// 3. A/B Compare Button
if (!html.includes('id="abStateBtn"')) {
  html = html.replace('<button id="globalBypassBtn"', '<div class="theme-picker" style="margin-left: 10px;"><button id="abStateBtn" class="sm-btn" style="background:#00d2d3; color:#09090b; font-weight:bold;">A/B: STATE A</button></div>\n      <button id="globalBypassBtn"');
}
...
console.log('Cleanly updated index.html');
